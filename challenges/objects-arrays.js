...
const universities = [];
const mapUniversities = graduates.map((arr) => {
  return arr.university;
})
mapUniversities.sort();
universities.push(mapUniversities);
...
const contactInfo = [];
const mapContactInfo = graduates.map((arr) => {
  return (arr.first_name + " " + arr.email)
})
contactInfo.push(mapContactInfo);
...
zooAnimals.filter((arr) => {
  lowerPopulation.push(arr.population < 5);
});
...
const reducePopulationTotal = zooAnimals.reduce((total, arr) => {
  return total += arr.population;
}, 0);
populationTotal.push(reducePopulationTotal)
